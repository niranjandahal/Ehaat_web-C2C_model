...
{
  global $conn;
  $query = "DELETE FROM products WHERE approved = 2";
...
function allnonapprovedproduct()
{
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo '<title>Non-Approved Products</title>';
    echo '<style>';
    echo 'body {';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      echo '<p>Category: ' . $row['product_category'] . '</p>';
      echo '<img class="product-image" src="../uploads/' . $row['product_image'] . '" alt="Product Image" />';
      echo '</div>';
...
function allnonapprovedproductsellername()
{
  ...
  echo '<meta charset="UTF-8">';
  echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">';
  echo '<title>Non-Approved Products by Seller</title>';
  echo '<style>';
  echo 'body {';
  echo '  font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif;';
  echo '  background-color: #f8f9fa;';
...
{
  global $conn;
  $query = "SELECT products.*, sellers.full_name as seller_name FROM products INNER JOIN sellers ON products.seller_id = sellers.id WHERE products.approved = 0 ORDER BY products.product_category ;";
  $result = mysqli_query($conn, $query);
  ...
  echo '<meta charset="UTF-8">';
  echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">';
  echo '<title>Non-Approved Products by Category</title>';
  echo '<style>';
  echo 'body {';
  echo '  font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif;';
  echo '  background-color: #f8f9fa;';
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
      echo '<div class="product-details">';
      echo '<h2>Category: ' . $row['product_category'] . '</h2>';
      echo '<h3>Product Name: ' . $row['product_name'] . '</h3>';
      echo '<p>Product Description: ' . $row['product_description'] . '</p>';
      echo '<p>Product Price: $' . $row['product_price'] . '</p>';
      echo '<p>Category: ' . $row['product_category'] . '</p>';
...
{
  global $conn;
  $query = "SELECT products.*, sellers.full_name AS seller_name FROM products INNER JOIN sellers ON products.seller_id = sellers.id WHERE products.approved = 1 ORDER BY products.seller_id DESC";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    echo '<meta charset="UTF-8">';
    echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">';
    echo '<title>Rejected Products</title>';
    echo '<style>';
    echo 'body {';
    echo '  font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif;';
    echo '  background-color: #f8f9fa;';
